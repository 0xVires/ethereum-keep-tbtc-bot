...
from config_testnet_private import WS_INFURA, MY_TELEGRAM_ID, TEL_TOKEN, TEL_URL
from config_testnet_public import (ETH_NOTIFICATION_LIMIT, KeepBonding, TBTCDepositToken, Hex_VendingMachine, TBTCSystem, 
...
w3 = Web3(Web3.WebsocketProvider(WS_INFURA))
    ...
    for event in bond_created_filter.get_all_entries():
        ...
        if subscriptions.get(operator):
            ...
            if bondedETH > ETH_NOTIFICATION_LIMIT:
                ...
                # Create tdts json
                with open("tdts_testnet.json", "r") as f:
                    tdts = json.load(f)
                if tdts.get(tdtID):
    ...
    for event in redemption_requested_filter.get_all_entries():
        ...
        if tdtID in tdts:
            ...
            for operator in tdts[tdtID]["operators"]:              
                ...
                with open("operator_tdts_testnet.json", "r") as f:
                    otdts = json.load(f)
                otdts[operator]["sumETH"] -= otdts[operator][tdtID]
                del otdts[operator][tdtID]
                with open("operator_tdts_testnet.json", "w") as f:
...
def main():
    ...
    with open('block_record_testnet.txt', 'r') as fh:
    ...
    try:
        ...
        with open('block_record_testnet.txt', 'w') as fh:
            fh.write(str(block))
    except Exception as ex:
        print(ex)
